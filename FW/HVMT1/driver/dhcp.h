...
//#define DHCP_DEBUG
#define HOST_NAME	"VINSSEN"		/**< Host Name */
#define	DHCP_WAIT_TIME		100
...
extern u_int getIP_DHCPS(unsigned char ch);								// Get the network configuration from the DHCP server
extern void check_DHCP_state(SOCKET s);							// Check the DHCP state
uint8 Set_Dhcp(unsigned char ch);
void set_DHCP_network(void);			/* Apply the leased IP address to LP-NetCAM II */
...
u_char GET_DNS_IP[4];	/**< DNS server ip address received from the DHCP server */
u_char GET_SIP[4];				/**< Local ip address received from the DHCP server */
